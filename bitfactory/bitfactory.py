...
class BFIntegerField(BFBasicDataType):
    ...
    def __init__(self, value=0, endian: BFEndian = BFEndian.LITTLE):
        if self.BYTE_WIDTH <= 0:
            raise BFTypeException(f"{type(self).__name__} must define a positive BYTE_WIDTH")
        ...
        if isinstance(val, int):
            ...
            if len(val) > self.BYTE_WIDTH:
                raise BFRangeException(
                    f"{type(self).__name__} accepts at most {self.BYTE_WIDTH} bytes, got {len(val)}"
                )
    ...
    def __setattr__(self, name, obj):
        if isinstance(obj, BFBasicDataType) and not name.startswith("_"):
            if hasattr(type(self), name):
                raise BFTypeException(f"{name!r} is reserved and cannot be used as a field name")
            self.add(name, obj)
        else:
    ...
    def pretty_print(self, indent: int = 0) -> str:
        ...
        for child in data_children:
            if isinstance(data_children[child], BFContainer):
                ...
                ret += "|" + data_children[child].pretty_print(indent + 1) + f" : {child} " + "\n"
        return ret
